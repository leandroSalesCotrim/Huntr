...
//     const frasesSujas = [
//         "ultimate edition", "game of the year", "special edition", "the game", "the complete edition", "the definitive edition", "the ezio collection", "single player", "return to arkham",
//     ]
...
//             // let authString = await AsyncStorage.getItem('authToken');
//             // if (authString) {
...
//     // popularDados();
// };
async function limparTagsRepetidas(): Promise<void> {
    try {
        const collectionName = 'jogos';
        const jogosCollection = collection(db, collectionName);
        ...
        querySnapshot.forEach(async (docSnapshot) => {
            ...
            if (data.trofeus) {
                // Para cada troféu, remover tags repetidas ou indesejadas
                console.log(data.trofeus)
                const trofeusAtualizados = data.trofeus.map((trofeu: any) => {
                    if (trofeu.tags) {
                        // Filtrar as tags para remover "Story", "Buggy" e "Example"
                        let tagsFiltradas = trofeu.tags.filter((tag: string) =>
                            tag !== "Story" && tag !== "Buggy" && tag !== "Example");

                        // Garantir que se houver tags repetidas de alguma forma, sejam removidas apenas uma vez
                        tagsFiltradas = [...new Set(tagsFiltradas)];

                        // Atualizar as tags do troféu
                        return {
                            ...trofeu,
                            tags: tagsFiltradas
                        };
                ...
                // Atualizar o documento no banco com os troféus alterados
                await updateDoc(doc(db, collectionName, docSnapshot.id), {
                    trofeus: trofeusAtualizados
                });

                console.log(`Documento ${docSnapshot.id} atualizado com troféus limpos.`);
            }
        });

        console.log('Processo de limpeza de tags concluído.');
    } catch (error) {
        console.error('Erro ao limpar as tags: ', error);
        throw error;
    }
}
limparTagsRepetidas();
